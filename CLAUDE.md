...
目标出画幅后按"中心→边缘"方向外推，在视野外围矩形路径搜索一圈后归位。

方向由**最后已知像素偏差**决定（每次收到坐标都记录，不受死区影响），不再依赖舵机运动历史。

```
TRACKING ──(50ms)──→ EXTRAPOLATE ──(超限)──→ PERIMETER ──(四角完成)──→ RETURN ──→ IDLE
    ↑                    ↑                       ↑                      ↑
    └──── 收到坐标即切回 TRACKING ───────────────┘                      │
    └────────────────── 收到坐标即切回 ─────────────────────────────────┘
```

**外推阶段**：沿 `last_err(x,y)` 方向持续移动，到达 `EXTRAPOLATE_MAX` 后进入边缘搜索。
**边缘搜索**：以外推终点和丢失点为对角构成矩形（加 `PERIM_PADDING` 扩展），顺序走完四角。

| 参数 | 默认值 | 作用 |
|------|--------|------|
| `LOST_THRESHOLD` | 5 | 连续无数据周期数(×10ms)才进入搜索 |
| `EXTRAPOLATE_MAX` | 50.0 | 外推最大偏离丢失点角度(°) |
| `EXTRAPOLATE_SPEED` | 0.8 | 外推每步角度(°) |
| `PERIM_PADDING` | 8.0 | 搜索矩形相对丢失点/外推终点的扩展量(°) |
| `PERIM_STEP` | 0.6 | 边缘搜索每步角度(°) |
| `DEFAULT_PERIM_RANGE` | 20.0 | 无方向信息时的默认搜索范围(°) |
| `RETURN_SPEED` | 1.2 | 归位每步角度(°) |

搜索过程中任何时刻收到新坐标，立即退出搜索、回到追踪模式。
